...
// Modified by Brankic (added window scroll to closest section and FadeOut/FadeIn)
/*******************************************************************************************/
    ...
    $.fn.pajinate = function(options){
		...
		var defaults = {
			...
			items_per_page : 10,			
			nav_panel_id : '.content .page_navigation',
		...
		return this.each(function(){
			...
			// Scroll the window to closest start of the section (added by Brankic)
			$nav_panels.find("a").click(function(e){
				//alert("vrci");
				var y = $nav_panels.closest("div.content-wrapper").offset().top;
				y = parseInt(y);
			...
			$page_container.find('.page_link').click(function(e){
				e.preventDefault();
				goto($(this).attr('longdesc'));//scrollToTop 
		...
		function goto(page_num){
			...
			// Fades the visible elements and then show current page of elements (added by Brankic)
			$items.parent().find("li:visible").fadeOut(function(){
				$items.slice(start_from, end_on).fadeIn();
			});
