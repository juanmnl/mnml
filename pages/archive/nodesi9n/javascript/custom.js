...
function where(section){
	...
	//alert(element_y + " " + element_y.length);
	var h_prev = 0;
	for(i = 1 ; i <= element_y.length ; i++ ){
		...
		if (offset_y > h_prev && offset_y < h){
			...
			if (!is_animated){
				...
				$("#menu ul li a[href=#" + section_id + "]").addClass("current");
			}
		}
		h_prev = h;
...
$(document).ready(function($){
	...
	if (offset_y > 100 && ($("#menu").hasClass("")) && (win_width > 1024) ){
...
/***************************************************
	 ADDITIONAL CODE FOR FOR PINNED NAVIGATION
***************************************************/
...
jQuery(document).ready(function($){
	...
	graph_init("services-graph2", 1000);
